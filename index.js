...
async function enterRoom(code){
  ...
  // Admin previews use receive-only offers and never become participants.
  previewsRef.on('child_added', snap => {
    const val = snap.val();
    if (!val || val.calleeId !== myId) return;
    answerPreview(snap.key, val);
  });
  previewsRef.on('child_removed', snap => closePreview(snap.key));
...
const previewPeers = {};
function answerPreview(previewId, preview){
  if (previewPeers[previewId]) return;
  const previewRef = previewsRef.child(previewId);
  const pc = new RTCPeerConnection(rtcConfig);
  previewPeers[previewId] = pc;
  localStream.getTracks().forEach(track => pc.addTrack(track, localStream));

  pc.onicecandidate = e => {
    if (e.candidate) previewRef.child('calleeCandidates').push(e.candidate.toJSON());
  };
  previewRef.child('callerCandidates').on('child_added', snap => {
    pc.addIceCandidate(new RTCIceCandidate(snap.val())).catch(console.error);
  });
  pc.setRemoteDescription(new RTCSessionDescription(preview.offer))
    .then(() => pc.createAnswer())
    .then(answer => pc.setLocalDescription(answer).then(() => answer))
    .then(answer => previewRef.child('answer').set({ type: answer.type, sdp: answer.sdp }))
    .catch(console.error);
}
function closePreview(previewId){
  const pc = previewPeers[previewId];
  if (!pc) return;
  pc.close();
  delete previewPeers[previewId];
}
...
function leaveRoom(){
  ...
  if (previewsRef){ previewsRef.off(); Object.keys(previewPeers).forEach(closePreview); }
  ...
  roomCode = null; roomRef = null; participantsRef = null; pairsRef = null; messagesRef = null; previewsRef = null;
